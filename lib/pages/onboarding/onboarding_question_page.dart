...
// ========== 调位置 / 换行 说明 ==========
// 1) 选项整体上/下：改 _kOptionsTopPadding（改小=整体上移）。块间距：改 _kBlockGap。
// 2) 单块高度按设计稿固定，小字区 _kOptionDescAreaHeight 足够 4～5 行，不压缩、不“框内下移”；不够就整区滚动。
// 3) 第一/三块左对齐、第二块右对齐，右缘统一：在 _OptionTile 里 isRightAligned 与 contentPadding 控制。
// 4) 换行：在 onboarding_v1_data.dart 里用 \n 换行。
// ========== 样式常量（大字 / 小字 分开） ==========
// ----- 大字（选项标题）------
const double _kOptionTitleSize = 17.0;
...
const double _kOptionTitleAreaHeight = 28.0;

// ----- 小字（选项描述，固定高度区域，在大字正下方）------
const double _kOptionDescSize = 14.0;
...
const double _kOptionTileVerticalPadding = 24.0;
...
/// 选项区整体距顶（改小=整体上移）
const double _kOptionsTopPadding = 8.0;
...
/// 选项区左右边距
const double _kOptionsHorizontalPadding = 40.0;
              ...
              child: SingleChildScrollView(
                ...
                child: Padding(
                  ...
                  child: Column(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      SizedBox(height: _kOptionsTopPadding),
                      ...List.generate(
                        item.options.length,
                        (i) => [
                          SizedBox(
                            height: _kOptionSlotHeight,
                            child: _OptionTile(
                          ...
                          if (i < item.options.length - 1)
                            SizedBox(
                              height: i == 0
                                  ? _kBlockGapAfterFirst
                                  : _kBlockGapAfterSecond,
                            ),
                        ],
                      ).expand((e) => e),
                    ],
...
class _OptionTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final EdgeInsets contentPadding = EdgeInsets.fromLTRB(
      horizontalInset + (isRightAligned ? 10 : 120),
      12,
