...
class AuthService:
    ...
    async def register(
        self, account: str, password: str, name: str
    ) -> Tuple[User, dict]:
        """
        注册新用户。
        ...
        existing = await self._get_user_by_account(account)
        if existing:
        ...
        logger.info("user_registered", user_id=str(user.id), account=user.email)
    ...
    async def forgot_password(self, account: str) -> Optional[str]:
        """
        ...
        user = await self._get_user_by_account(account)
        if not user:
            logger.info("forgot_password_account_not_found", account=account)
            return None
    ...
    async def _get_user_by_account(self, account: str) -> Optional[User]:
        """按账号（users.email 列存登录名）查找未删除用户"""
        result = await self.db.execute(
            select(User).where(
                User.email == account.strip().lower(),
                User.is_deleted == False,
            )
        )
    ...
    async def _get_user_by_email(self, email: str) -> Optional[User]:
        """兼容旧调用"""
        return await self._get_user_by_account(email)
